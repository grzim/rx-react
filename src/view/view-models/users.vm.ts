...
export const getUsersViewModel = (usersService: UserService, filersService: FilterService) => {
  const {addUser, deleteUser, users$} = usersService;
  const {setAgeFilter, ageFilterLabel$, filters$, setNameFilterValue} = filersService;
  const [onSetAgeFilter, onSetNameFilter] = [setAgeFilter, setNameFilterValue].map(subjectToChange)
  ...
  return function useViewModel() {
    const [users = [], filterLabel] = useSubscriptions(filteredUsers$, ageFilterLabel$);
    return {
      ...nextify({
        addUser,
        deleteUser,
