import { combineLatestWith, debounceTime, map, merge, scan, startWith, Subject } from 'rxjs';
...
export const getFilterService = () => {
  ...
  const nameFilterValue$ = setNameFilterValue.pipe(
    startWith(''),
    map((value: string): Filterable =>  (item) => item.name.includes(value)))

  const setAgeFilter = new Subject<AgeFilter>();
  const removeAgeFilter = new Subject<AgeFilter>();
  const ageFilterLabel$ = setAgeFilter.pipe(
    startWith('all' as AgeFilter))

  const ageFilter$ = merge(
    setAgeFilter.pipe(map(labelToFn)),
    removeAgeFilter.pipe(map(() => ageFilter.all))).pipe(
    startWith(ageFilter.all)
  )

  const filters$ = ageFilter$.pipe(
    combineLatestWith(nameFilterValue$),
  )
  ...
  const filterHistory$ = merge(
    ...
    scan((acc: string[], curr, i) => [...acc, `${i+1}: ${curr || 'filter removed'}`].slice(-3), []),
  )

  return {
    filters$, ageFilterLabel$, filterHistory$, removeAgeFilter, setAgeFilter, setNameFilterValue
  }
}
